...
let handler = async (m, { conn, text }) => {
  ...
  if (m.chat in conn.hartatahta) throw 'There are still those who are making\Treasure of Thrones\text in this chat...wait for it to finish'
  else conn.hartatahta[m.chat] = true
  m.reply('_is making..._\n*Please wait about 1 minute*')
  try {
    let img = await ht(text ? text : ':v')
    conn.sendFile(m.chat, img, 'Treasure Tahta.png', '*© Toshiro*\nMade with FFmpeg', m)
    setTimeout(() => {
      fs.unlinkSync(img)
    }, 5000);
  } finally {
